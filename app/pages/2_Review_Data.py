...
def _parse_date(df: pd.DataFrame, meta: dict) -> Tuple[pd.DataFrame, str]:
    """
    Parse the date field indicated in metadata["data"]["date_field"] (default "DATE").
    - Resolves the column name case-insensitively against df.columns.
    - Coerces to pandas datetime and drops timezone info.
    - Sorts ascending by the resolved date column.
    Returns (df, resolved_date_col_name).
    """
    wanted = str(meta.get("data", {}).get("date_field") or "DATE")

    # Resolve against df.columns case-insensitively (prefer exact, else exact-lower match)
    cols = list(map(str, df.columns))
    if wanted in cols:
        date_col = wanted
    else:
        lower_map = {c.lower(): c for c in cols}
        date_col = lower_map.get(wanted.lower(), wanted)  # fall back to wanted if missing
...
def kpi_grid_fixed(boxes: list, per_row: int = 3):
    """
    Same as kpi_grid but pads with empty tiles so each row has the same width.
    """
    if not boxes:
        return
    # pad to multiple of per_row
...
def load_data_from_gcs(
    ...
    data_blob = (
        _data_latest_blob(country)
        if data_ts == "Latest"
        else _data_blob(country, str(data_ts))
    )
    meta_blob = (
        _meta_latest_blob(country)
        if meta_ts == "Latest"
        else _meta_blob(country, str(meta_ts))
    )
...
if "df" not in st.session_state or "meta" not in st.session_state:
    try:
        df, meta, date_col = load_data_from_gcs(
            GCS_BUCKET,
...
paid_spend_cols = [
    c for c in _mapping.get("paid_media_spends", []) if c in df.columns
]
paid_var_cols = [
    c for c in _mapping.get("paid_media_vars", []) if c in df.columns
]
...
if not goal_cols and isinstance(meta, pd.DataFrame) and not meta.empty:
    ...
    if _COL and _CAT:
        ...
        _m_goal2 = _mt.loc[
            _mt[_CAT].str.lower().eq("secondary_goal"), _COL
        ].tolist()
    ...
    m = (
        pd.concat(
            [
                m,
                mt[
                    ["column_name", "main_category", "platform", "display_name"]
                ],
            ],
            axis=0,
        )
...
def _pick(
    df_: pd.DataFrame, wanted: str, default: str | None = None
) -> str | None:
    """
...
def filter_range(d: pd.DataFrame) -> pd.DataFrame:
    if RANGE == "all":
        return d
    if RANGE == "2y":
        return d[d[DATE_COL] >= (date_max - pd.DateOffset(years=2))]
    if RANGE == "ly":
...
def previous_window(
    full_df: pd.DataFrame, current_df: pd.DataFrame
) -> pd.DataFrame:
    """
...
with tab_load:
    ...
    versions = (
        _list_country_versions(GCS_BUCKET, country) if country else ["Latest"]
    )
    data_ts = c2.selectbox(
        "Data version", options=versions, index=0, key="picked_data_ts"
    )
    meta_ts = c3.selectbox(
        "Metadata version", options=versions, index=0, key="picked_meta_ts"
    )
...
# =============================
# TAB 3 — MARKETING OVERVIEW (fixed totals + single-channel section)
# =============================
with tab_mkt:
    ...
    # fixed card widths via padding
    kpi_grid_fixed(spend_boxes, per_row=4)
    st.markdown("---")

    # ----- Change vs Previous — Waterfall (always all platforms) -----
    ...
    # ----- Channel Mix (stacked, always all platforms) -----
    ...
    # ----- Single Channel View (selector + spend over time + funnel) -----
    st.markdown("#### Single Channel View")

    # Choose default: highest current spend
    sel_platform = None
    if not plat_map_df.empty and not df_r.empty:
        long_sp_tmp = (
            df_r.melt(
        ...
        cur_by_p_tmp = long_sp_tmp.groupby("platform")["spend"].sum().sort_values(ascending=False)
    ...
    if sel_platform:
        # Spend over time for selected channel (fixes the KeyError)
        sp_cols = _spend_cols_for_platform(sel_platform)
        if sp_cols:
            # build a single spend series before resetting — avoids missing columns on ts
            sp_series = df_r.set_index(DATE_COL)[sp_cols].sum(axis=1)
            ts = (
                sp_series
                .resample(RULE)
                .sum(min_count=1)
            ...
            ts.rename(columns={sp_series.name: "spend"}, inplace=True)  # ensure column name
            ts["PERIOD_LABEL"] = period_label(ts["DATE_PERIOD"], RULE)

            fig_sc = go.Figure(
                go.Bar(x=ts["PERIOD_LABEL"], y=ts["spend"], name=f"{sel_platform} Spend")
            )
            fig_sc.update_layout(
                title=f"{sel_platform} {spend_label} Over Time — {TIMEFRAME_LABEL}, {agg_label}",
                xaxis=dict(title="Date", title_standoff=8),
        ...
        spend_cols_for_plat = _spend_cols_for_platform(sel_platform)
        ...
        with col_left:
            st.markdown(f"**{sel_platform} Funnel**")
            steps = []
        ...
        st.info("Platform mapping not available for the single-channel view.")
...
with tab_rel:
    ...
    if not goal_cols:
        ...
        driver_all = sorted(
            list(
                dict.fromkeys([c for c in all_driver_cols if c in df_r.columns])
            )
        )
        if not driver_all:
            ...
            if len(x_f) < 5:
                st.info(
                    "Not enough data points after filtering to fit a curve."
                )
            else:
                # Fit quadratic
                ...
                def classify_r2_card(val: float):
                    if val is None or not np.isfinite(val):
                        return (
                            "yellow",
                            "Insufficient data — treat with caution.",
                        )
                    if val >= 0.35:
                        return (
                            "green",
                            "Promising signal — relationship likely meaningful; candidate for MMM.",
                        )
                    if val >= 0.15:
                        return (
                            "yellow",
                ...
                def classify_mae_card(val: float):
                    ...
                    if val <= 0.10:
                        return (
                            "green",
                            "Average error is small vs goal scale — usable for exploration.",
                        )
                    if val <= 0.30:
                        return (
                            "yellow",
                            "Average error is moderate — interpret cautiously.",
                        )
                    return (
                        "red",
                        "Average error is large — not reliable for exploration.",
                    )
                ...
                def classify_rho_card(val: float):
                    ...
                    if strength >= 0.15:
                        return (
                            "yellow",
                            "Some monotonic pattern — consider with caution.",
                        )
                    return "red", "Weak/none — ranks move inconsistently."

                TITLE_R2, TITLE_MAE, TITLE_RHO = (
                    "R²",
                    "MAE (relative)",
                    "Spearman ρ",
                )
                ...
                def colored_note(level: str, text: str):
                    base = SCORE_COLORS[level]
                    bg = hex_to_rgba(base, 0.10)
                    brd = hex_to_rgba(base, 0.35)
    ...
    .tooltip-wrap .tooltip-text::after {
      ...
      border-color: #111 transparent transparent transparent;
                ...
                def score_bar_metric_first(
                    ...
                    lvl_rho, msg_rho = classify_rho_card(
                    ...
                    rho_txt = (
                        f"{rho:+.2f}" if np.isfinite(rho) else "—"
                    )  # keep sign
                    ...
                    with c3:
                        score_bar_metric_first(
                            "Spearman ρ", TIP_RHO, rho_txt, lvl_rho, p_rho
                        )
                        if np.isfinite(rho):
                            direction = (
                                "positive"
                                if rho > 0
                                else ("negative" if rho < 0 else "no")
                            )
                            colored_note(
                                lvl_rho,
                    ...
                    mr_tbl = pd.DataFrame(
                        {
                            ...
                            "Marginal return (dy/dx)": [
                                f"{float(v):.4f}" for v in dydx
                            ],
                        }
                    )
                    st.dataframe(
                        mr_tbl, hide_index=True, use_container_width=True
                    )
