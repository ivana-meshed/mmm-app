...
display_map, nice, goal_cols_init, mapping_init, m, ALL_COLS_UP, IMPR_COLS, CLICK_COLS, SESSION_COLS, INSTALL_COLS = build_meta_views(meta, df)

# ---- Sidebar (page-local controls) ----
GOAL, sel_countries, TIMEFRAME_LABEL, RANGE, agg_label, FREQ = render_sidebar(meta, df, nice, goal_cols_init)
...
goal_cols = [g["var"] for g in (meta.get("goals") or []) if g.get("var") in df.columns]
dep_type  = (meta.get("dep_variable_type") or {})
goal_labels = {g: f'{g} [{dep_type.get(g, "").strip()}]'.strip(" []") for g in goal_cols}
...
with tab_diag:
    ...
    def _prepare_X(frame: pd.DataFrame, cols: list) -> pd.DataFrame:
        if frame.empty or not cols:
            return pd.DataFrame()
        X = frame[ [c for c in cols if c in frame.columns] ].apply(pd.to_numeric, errors="coerce")
    ...
    def _bucket_map():
        return {
            "Paid Spend":     [c for c in (paid_spend_cols  or []) if c in df_r.columns and c not in goals_to_exclude],
            "Paid Media Vars":[c for c in (paid_var_cols    or []) if c in df_r.columns and c not in goals_to_exclude],
            "Organic Vars":   [c for c in (organic_cols     or []) if c in df_r.columns and c not in goals_to_exclude],
            "Context Vars":   [c for c in (context_cols     or []) if c in df_r.columns and c not in goals_to_exclude],
        }
    ...
    def _all_drivers():
        ...
        flat = []
        for k in ["Paid Spend","Paid Media Vars","Organic Vars","Context Vars"]:
            flat.extend(b.get(k, []))
    ...
    with st.expander("Configurations", expanded=False):
        countries_all = sorted(df_r["COUNTRY"].dropna().astype(str).unique()) if "COUNTRY" in df_r else []
    ...
    htip("Cond#", "Standardized condition number σ_max/σ_min (SVD). Lower is better: <15 low, 15–30 medium, >30 high.")
    ...
    for ctry in sel_ctry:
        dct = (df_r[df_r["COUNTRY"].astype(str).eq(ctry)] if "COUNTRY" in df_r else df_r.copy())
    ...
    st.dataframe(
        disp_all[["Country","#Vars","Cond#","CondBand",f"VIF>{vif_thr}","PCs @ 80%","PCA Band"]],
        use_container_width=True,
    )
    ...
    default_nice = [nice(c) for c in st.session_state["diag_selected_drivers_v10"] if c in nice_to_raw.values()]
    ...
    st.session_state["diag_selected_drivers_v10"] = [nice_to_raw[n] for n in sel if n in nice_to_raw]
    ...
    if drivers_sel:
        d_comb = (df_r[df_r["COUNTRY"].astype(str).isin(sel_ctry)] if "COUNTRY" in df_r else df_r.copy())
    ...
    if drivers_sel:
        if strong_to_drop:
            if st.button(f"Drop suggested: Strong (≥10) — {len(set(strong_to_drop))} vars"):
                st.session_state["diag_selected_drivers_v10"] = [c for c in st.session_state["diag_selected_drivers_v10"] if c not in set(strong_to_drop)]
                st.rerun()
        if strong_to_drop or mod_to_drop:
            ...
            if st.button(f"Drop suggested: Strong + Moderate (≥7.5) — {total_sm} vars"):
                ...
                st.session_state["diag_selected_drivers_v10"] = [c for c in st.session_state["diag_selected_drivers_v10"] if c not in to_drop]
                st.rerun()
    ...
    if drivers_sel:
        ...
        st.dataframe(
            disp2[["Country","#Vars","Cond#","CondBand",f"VIF>{vif_thr}","PCs @ 80%","PCA Band"]],
            use_container_width=True,
        )
    ...
    st.dataframe(
        detail_disp[["Country","#Vars","Cond#","CondBand",f"VIF>{vif_thr}","PCs @ 80%","PCA Band"]],
        use_container_width=True,
    )
...
with tab_deep:
    ...
    driver_all = _dedup(paid_spend_cols + paid_var_cols + organic_cols + context_cols + factor_cols + other_driver_cols)
