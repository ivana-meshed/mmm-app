...
def _safe_tick_once(
    ...
    logger.info("=" * 80)
    logger.info(
        f"[QUEUE_TICK_START] _safe_tick_once called for queue: {queue_name}"
    )
    ...
    for attempt in range(max_retries):
        ...
        if not blob.exists():
            ...
            logger.warning(
                f"[QUEUE_TICK] Blob still doesn't exist after create attempt, retrying..."
            )
            continue
        ...
        try:
            ...
            logger.info(
                f"[QUEUE_TICK] Loaded queue document, generation: {gen}"
            )
        except Exception as e:
            logger.warning(
                f"[QUEUE_TICK] Failed to parse queue document: {e}, using init doc"
            )
            doc = _init_doc()
        ...
        logger.info(
            f"[QUEUE_TICK_FOUND] Country: {q[pend_idx].get('params', {}).get('country')}"
        )
        ...
        logger.info(
            f"[QUEUE_TICK] Leasing job {entry.get('id')} (setting status to LAUNCHING)"
        )
        ...
        try:
            ...
            logger.info(
                f"[QUEUE_TICK] Successfully leased job {entry.get('id')}"
            )
        except PreconditionFailed:
            # Another process leased it; retry from the top
            logger.info(
                f"[QUEUE_TICK] Lost lease race for job {entry.get('id')}, retrying..."
            )
            continue
        ...
        logger.info(
            f"[QUEUE] Job params: country={entry.get('params', {}).get('country')}, "
            f"revision={entry.get('params', {}).get('revision')}, "
            f"iterations={entry.get('params', {}).get('iterations')}"
        )
        logger.info(
            f"[QUEUE_BEFORE_LAUNCHER] About to call launcher function..."
        )
        logger.info(f"[QUEUE_BEFORE_LAUNCHER] Launcher type: {type(launcher)}")
        logger.info(
            f"[QUEUE_BEFORE_LAUNCHER] Params keys: {list(entry.get('params', {}).keys())}"
        )
        try:
            logger.info(f"[QUEUE_LAUNCHER_CALL] Calling launcher NOW...")
            exec_info = launcher(entry["params"])
            logger.info(
                f"[QUEUE_LAUNCHER_RETURNED] Launcher returned successfully!"
            )
            logger.info(
                f"[QUEUE_LAUNCHER_RETURNED] Exec info keys: {list(exec_info.keys() if exec_info else [])}"
            )
            ...
            logger.error(
                f"[QUEUE_ERROR] LAUNCH FAILURE - Job {entry.get('id')}"
            )
            logger.error(
                f"[QUEUE_ERROR] ========================================"
            )
...
class CloudRunJobManager:
    ...
    def get_execution_status(self, execution_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(
                f"Error getting execution status for '{execution_name}': {e}",
                exc_info=True,
            )
            error_msg = str(e)
...
def handle_queue_tick_from_query_params(
    ...
    logger.info("=" * 80)
    logger.info(
        f"[QUEUE_TICK_ENTRY] ========== QUEUE TICK ENDPOINT CALLED =========="
    )
    ...
    logger.info(
        f"[QUEUE_TICK_ENTRY] Launcher type: {type(launcher) if launcher else None}"
    )
    logger.info("=" * 80)
    ...
    try:
        ...
        logger.error("=" * 80)
        logger.error(
            f"[QUEUE_TICK_ERROR] ========== QUEUE TICK HANDLER FAILED =========="
        )
...
def require_login_and_domain(allowed_domain: Optional[str] = None) -> None:
    ...
    if allowed_domain is not None:
        ...
        allowed_domains = (
            ALLOWED_DOMAINS if ALLOWED_DOMAINS else ["mesheddata.com"]
        )
    ...
    if not is_logged_in:
        ...
        if st.button("Sign in with Google"):
            st.login()  # type: ignore # uses providers list from [auth] config
        st.stop()
    ...
    if not any(email_domain == domain for domain in allowed_domains):
        ...
        if len(allowed_domains) == 1:
            st.error(
                f"This app is restricted to @{allowed_domains[0]} accounts."
            )
        else:
            domains_str = ", ".join(f"@{d}" for d in allowed_domains)
            st.error(
        ...
        if st.button("Sign out"):
            st.logout()  # type: ignore # uses providers list from [auth] config
        st.stop()
