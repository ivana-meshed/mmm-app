...
# ─────────────────────────────
# Jobs JOB_HISTORY (source of truth)
# ─────────────────────────────
st.markdown("## 🗃️ Jobs JOB_HISTORY (GCS)")
jobs_job_history_object = st.text_input(
    "JOB_HISTORY object path (inside bucket, not gs://)",
    value=os.getenv("JOBS_JOB_HISTORY_OBJECT", "robyn-jobs/job_history.csv"),
    key="job_history_obj",
)
if st.button("Load job_history"):
    try:
        client = storage.Client()
        b = client.bucket(st.session_state["gcs_bucket"])
        blob = b.blob(jobs_job_history_object)
        if not blob.exists():
            st.warning(
                "JOB_HISTORY file not found; it will be created on first append."
            )
        else:
            data = blob.download_as_bytes()
            import io

            df_job_history = pd.read_csv(io.BytesIO(data))
            st.session_state["df_job_history"] = df_job_history
            st.success(f"Loaded {len(df_job_history)} rows.")
    except Exception as e:
        st.error(f"JOB_HISTORY load failed: {e}")
if "df_job_history" in st.session_state:
    st.dataframe(st.session_state["df_job_history"], use_container_width=True)
with st.expander("➕ Add PLANNED row to job_history"):
    l_country = st.text_input(
        "Country (job_history)", value=country, key="l_country"
    )
    l_revision = st.text_input(
        "Revision (job_history)", value=revision, key="l_revision"
    )
    l_note = st.text_input("Note/Message", value="", key="l_note")
    if st.button("Append to job_history as PLANNED"):
        try:
            client = storage.Client()
            b = client.bucket(st.session_state["gcs_bucket"])
            blob = b.blob(jobs_job_history_object)
            ...
            st.success("Row appended to job_history.")
        except Exception as e:
