...
class HealthChecker:
    ...
    def check_container_health(self):
        ...
        try:
            ...
            health_status["checks"]["disk_usage"] = psutil.disk_usage(
                "/"
            ).percent
            ...
            if not all(
                health_status["checks"].get(check, False)
...
def create_health_page():
    ...
    if (
        st.button("🔄 Refresh Health Status")
        or "auto_refresh" not in st.session_state
    ):
    ...
    with col1:
        st.metric(
            "CPU Usage", f"{health_status['checks'].get('cpu_usage', 0):.1f}%"
        )
        st.metric(
            "Memory Usage",
            f"{health_status['checks'].get('memory_usage', 0):.1f}%",
        )

    with col2:
        st.metric(
            "Disk Usage", f"{health_status['checks'].get('disk_usage', 0):.1f}%"
        )
    ...
    with col3:
        ...
        gcs_status = (
            "✅" if health_status["checks"].get("gcs_authenticated") else "❌"
        )
    ...
    for component, status in warm_status.items():
        icon = "✅" if status else "⏳"
        st.write(
            f"{icon} {component.replace('_', ' ').title()}: {'Ready' if status else 'Not Ready'}"
        )
    ...
    st.write("---")
    st.write(
        "Page will auto-refresh every 30 seconds when container is warming up"
    )
