...
class TestExperimentHelpers(unittest.TestCase):
    ...
    def test_build_job_config_basic(self, mock_st):
        ...
        self.assertEqual(config["paid_media_vars"], ["GA_IMPRESSIONS", "META_IMPRESSIONS"])
    ...
    def test_metadata_structure(self):
        ...
        metadata = {
            "goals": [
                ...
                {"var": "CONVERSIONS", "group": "primary", "type": "conversion"},
                {"var": "LEADS", "group": "secondary", "type": "conversion"}
            ],
            "mapping": [
                {"var": "GA_COST", "category": "paid_media_spends", "channel": "ga"},
                {"var": "GA_IMPRESSIONS", "category": "paid_media_vars", "channel": "ga"},
                {"var": "IS_WEEKEND", "category": "context_vars", "channel": ""},
            ]
        }
        
        # Test goals extraction
        primary_goals = [g["var"] for g in metadata["goals"] if g["group"] == "primary"]
        self.assertIn("REVENUE", primary_goals)
        self.assertIn("CONVERSIONS", primary_goals)
        self.assertNotIn("LEADS", primary_goals)
        
        # Test type mapping
        revenue_goal = next(g for g in metadata["goals"] if g["var"] == "REVENUE")
        self.assertEqual(revenue_goal["type"], "revenue")
        
        # Test variable mapping
        paid_spends = [m["var"] for m in metadata["mapping"] if m["category"] == "paid_media_spends"]
