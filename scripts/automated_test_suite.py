...
class OptimizationTestSuite(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.production_url = os.getenv(
            "PRODUCTION_URL", "https://mmm-app-wuepn6nq5a-ew.a.run.app"
        )
    ...
    def test_06_performance_benchmark(self):
        ...
        logger.info(
            f"✅ Performance benchmark passed: {duration_minutes:.1f} minutes"
        )
    ...
    def test_07_error_handling_and_recovery(self):
        ...
        response = requests.post(
            f"{self.production_url}/train", json=invalid_payload, timeout=30
        )
        ...
        health_response = requests.get(
            f"{self.production_url}/health", timeout=10
        )
    ...
    def test_08_cost_efficiency_validation(self):
        ...
        self.assertGreater(
            cpu_usage, 5, "CPU usage too low - resources may be wasted"
        )
        self.assertLess(
            cpu_usage, 85, "CPU usage too high - may need more resources"
        )
    ...
    def test_10_rollback_capability(self):
        ...
        self.assertTrue(
            os.path.exists(rollback_script), "Rollback script not found"
        )
...
class OptimizationTestRunner:
        ...
        class DetailedTestResult(unittest.TestResult):
            ...
            def stopTest(self, test):
                ...
                if test._testMethodName in [
                    f[0]._testMethodName for f in self.failures
                ]:
                    status = "FAIL"
                elif test._testMethodName in [
                    e[0]._testMethodName for e in self.errors
                ]:
                    status = "ERROR"
    ...
    def generate_test_report(self, result):
        ...
        passed_tests = len(
            [t for t in result.test_results if t["status"] == "PASS"]
        )
        failed_tests = len(
            [t for t in result.test_results if t["status"] == "FAIL"]
        )
        error_tests = len(
            [t for t in result.test_results if t["status"] == "ERROR"]
        )
...
def main():
    ...
    parser.add_argument(
        "--production-url", help="Production environment URL (for comparison)"
    )
    parser.add_argument(
        "--output", default="test_report.md", help="Test report output file"
    )
    ...
    runner = OptimizationTestRunner(
        ...
        or "https://mmm-app-wuepn6nq5a-ew.a.run.app"
    )
