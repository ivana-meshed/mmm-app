        ...
        if response.status_code == 200:
            ...
            if os.path.exists("/tmp/warming_status.json"):
                ...
                successful_components = sum(
                    1
                    for status in warming_status["warming_status"].values()
                    if status
                )
                ...
                if successful_components >= 3:  # At least 3 of 4 components
                    ...
                    print(
                        "❌ Container warming test FAILED - insufficient components warmed"
                    )
                    return False
            else:
                print(
                    "⚠️ Warming status file not found - container may not be fully warmed"
                )
                return False
        else:
