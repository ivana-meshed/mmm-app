...
def list_model_runs_needing_extraction(
    bucket_name, country=None, revision=None
):
    """
    ...
    logger.info(
        f"Scanning GCS bucket '{bucket_name}' with prefix '{prefix}'..."
    )
        ...
        if blob.name.endswith("OutputCollect.RDS"):
            ...
            if country and not revision:
                ...
                if len(path_parts) >= 3:
                    ...
                    logger.debug(
                        f"Skipping country filter for unexpected path: {run_prefix}"
                    )
    ...
    logger.info(
        f"Found {len(model_runs)} model runs needing parquet extraction"
    )
    return model_runs
...
def main():
    ...
    parser.add_argument(
        "--limit", type=int, help="Maximum number of runs to process"
    )
    ...
    runs_to_process = list_model_runs_needing_extraction(
        args.bucket, country=args.country, revision=args.revision
    )
