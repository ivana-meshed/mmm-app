...
2. Generates comprehensive benchmark — default **9 variants** (sequential: each of 4 dimensions varied independently: adstock, splits, time_agg, spend_var).
   Use `--cartesian` for 18 combinations (product of all dimensions), `--all-adstock` for all 3 adstock types, `--all-windows` to add a window sweep.
...
- Plots: `./benchmark_analysis/*.png` (6 core plots; enrichment plots when decomp data is available)
...
### Default Sequential Benchmark (geometric adstock only)

Sequential mode varies one dimension at a time — 9 variants (default):
...
### Cartesian Benchmark (`--cartesian`, geometric adstock only)
...
### Full Cartesian Benchmark (`--cartesian --all-adstock`, all 3 adstock types)
...
- `comprehensive_benchmark.json` - Cartesian combinations (generic dataset)
- `comprehensive_benchmark_fleet_marketplace.json` - Fleet/mobility marketplace benchmark: 5 dimensions
  (adstock × train_splits × time_aggregation × spend_var_mapping × seasonality_window). Default
  geometric adstock gives 30 variants (1 × 3 × 2 × 5 × 1 — `full` window only). Use `--all-adstock`
  for 90 variants, `--all-windows` to add 2y/3y training windows, or both for 270 variants. The
  `seasonality_window` variants use `weeks_back` offsets resolved at runtime against `end_date`.
- `channel_type_assignments_fleet_marketplace.json` - Channel-type assignments for the fleet
  marketplace dataset (referenced via `channel_type_assignments_config`)
- `generic_hyperparameter_ranges_v2.json` - Per-channel, per-frequency hyperparameter ranges
  (referenced by other configs via `hyperparameter_ranges_config`); supports 5 presets:
  `conservative`, `balanced`, `exploratory`, `fb`, `meshed`
- `channel_type_assignments.json` - Maps variable names to channel types (referenced via
