...
gcs_put <- function(local_file, object_path, upload_type = c("simple","resumable")) {
  ...
  googleCloudStorageR::gcs_upload(file=lf, name=object_path, bucket=bkt,
...
safe_parse_numbers <- function(df, cols) {
  for (cl in intersect(cols, names(df))) {
    x <- df[[cl]]
    if (is.character(x))      df[[cl]] <- readr::parse_number(x)
    else if (is.factor(x))    df[[cl]] <- readr::parse_number(as.character(x))
    else if (is.numeric(x))   df[[cl]] <- as.numeric(x)
    else                      df[[cl]] <- suppressWarnings(readr::parse_number(as.character(x)))
  }
  df
}
...
if ("DATE" %in% names(df)) {
  ...
  df$date <- as.Date(df[["date"]]); df[["date"]] <- NULL
} else stop("No DATE/date column in data")
...
df <- df %>% mutate(
  ...
  ORGANIC_TRAFFIC = rowSums(select(., any_of(c("NL_DAILY_SESSIONS","SEO_DAILY_SESSIONS","DIRECT_DAILY_SESSIONS",
                                               "TV_DAILY_SESSIONS","CRM_OTHER_DAILY_SESSIONS","CRM_DAILY_SESSIONS"))), na.rm=TRUE),
...
if (file.exists(ann_path)) {
  ...
  names(ann) <- toupper(names(ann)); ann$DATE <- as.Date(ann$DATE)
  ...
  if (length(flag_cols))
    df <- df %>% mutate(across(all_of(flag_cols), ~ {
      v <- dplyr::case_when(. %in% c(TRUE,"TRUE","True",1,"1") ~ 1L,
...
keep_idx <- vapply(seq_along(paid_media_spends), function(i) {
  sc <- paid_media_spends[i]; sum(df[[sc]], na.rm=TRUE) > 0
}, logical(1))
...
if (should_add_n_searches(df, paid_media_spends) && "N_SEARCHES" %in% names(df))
  organic_vars <- unique(c(org_base, "N_SEARCHES")) else organic_vars <- org_base
...
# plot allocator
alloc_dir <- file.path(dir_path, paste0("allocator_plots_", timestamp)); dir.create(alloc_dir, showWarnings=FALSE)
png(file.path(alloc_dir, paste0("allocator_", best_id, "_365d.png")), width=1200, height=800)
plot(AllocatorCollect); dev.off()
